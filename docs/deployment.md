...
Compose binds the application to `127.0.0.1` by default. Set `ATOM_BIND_ADDRESS` only when another interface must accept traffic. For an internet-facing deployment, set strong `RF_API_KEY` and `BUILDINGS_API_KEY` values and place a trusted proxy or gateway in front of port 8080 for:
