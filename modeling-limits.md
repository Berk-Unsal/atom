...
The network engine dispatches explicit propagation models. `urban_short_range` is the deterministic 3GPP UMa LOS/NLOS baseline for 2.6/28 GHz, `legacy_fspl_walls` is the compatibility model and fallback, and `research_sub_thz` is the explicitly research-only 140 GHz profile. The urban baseline uses 2D footprint geometry for LOS/NLOS classification, known Tx/Rx heights, and no uncertain building height. Its applicability and any fallback reason are returned in `rf_contract` and ray explainability fields. The separate [Concept 4E building-entry analysis](concept-4e-building-entry.md) adds one standard O2I facade term at zero indoor depth; it is not indoor or whole-building coverage. The point-to-point path profile remains an isolated advanced diagnostic with model id `path-profile-diagnostic-v1`. Read the [Concept 4D design note](concept-4d-urban-propagation.md) for equations and limits.
